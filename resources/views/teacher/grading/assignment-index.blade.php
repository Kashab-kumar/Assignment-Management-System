...
@section('page-title', 'Assignment Grading')
...
<div class="page-shell">
    <div class="panel hero">
        <div class="crumbs"><a href="{{ route('teacher.courses.show', $course) }}">{{ $course->name }}</a> / Assignment Grading</div>
        <h1>Assignment Grading</h1>
        <p>Search, filter, and manage submitted assignments for this course.</p>
    </div>
    ...
    <div class="panel">
        <form method="GET" class="filters">
            <div>
                ...
                <input id="search" name="search" value="{{ $search }}" placeholder="Search assignment title">
            </div>
            <div>
            ...
            <div class="actions">
                ...
                <a href="{{ route('teacher.courses.assignment-grading', $course) }}" class="btn btn-secondary">Reset</a>
            </div>
        </form>
    </div>
    ...
    <div class="panel table-wrap">
            ...
            <table>
                ...
                <tbody>
                    @foreach($assignments as $row)
                        @php $assignment = $row['model']; @endphp
                        <tr>
                            <td>
                                <div class="title">{{ $assignment->title }}</div>
                                <div class="meta">{{ $assignment->type ? ucfirst($assignment->type) : 'Assignment' }}</div>
                            </td>
                            ...
                            <td>
                                <a href="{{ route('teacher.grading.assignments.submissions', $assignment) }}" class="btn btn-primary">View Submissions</a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
        ...
        @else
            <div class="empty">
                <h3 style="margin:0 0 8px; color:#0f172a;">No assignments found</h3>
