...
@section('title', $exam->title . ' Submissions')
@section('page-title', $exam->title . ' Submissions')
...
<div class="page-shell">
    <div class="panel hero">
        <div style="display:flex; justify-content:space-between; gap:16px; align-items:flex-start; flex-wrap:wrap;">
            <div>
                <h1>{{ $exam->title }}</h1>
                <p>Student answer sheets for this exam.</p>
                <div class="meta">
                    <span>{{ ucfirst($exam->type) }}</span>
                    <span>{{ optional($exam->exam_date)->format('d M Y') ?? '-' }}</span>
                    <span>Max Score {{ $exam->max_score }}</span>
                    <span>{{ $exam->module?->title ?? 'General' }}</span>
                </div>
            </div>
            <a href="{{ route('teacher.courses.exam-grading', $exam->course) }}" class="btn btn-secondary">Back to Grading</a>
        </div>
    </div>
    ...
    <div class="panel">
        <form method="GET" class="toolbar">
            ...
            <div>
                ...
                <a href="{{ route('teacher.grading.exams.submissions', $exam) }}" class="btn btn-secondary">Reset</a>
            </div>
        </form>
    </div>
    ...
    <div class="panel table-wrap">
                        ...
                        <tr>
                            ...
                            <td>
                                @if($row['status'] !== 'missing')
                                    <a href="{{ route('teacher.grading.exam-students.show', [$exam, $student]) }}" class="btn btn-primary">Grade</a>
                                @else
        ...
        @else
            <div class="empty">
                <h3 style="margin:0 0 8px; color:#0f172a;">No matching exam submissions</h3>
